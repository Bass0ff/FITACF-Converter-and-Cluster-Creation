...
class RangeTimePlot(object):
    ...
    def addClusterPlot(self, data_dict, clust_flg, beam, title, show_closerange=True, xlabel=''):
        ...
        # time = np.concatenate([arr for arr in data_dict['time']])
        # gate = np.concatenate([arr for arr in data_dict['gate']])
        # allbeam = np.concatenate([arr for arr in data_dict['beam']])
        # flags = np.concatenate([arr for arr in clust_flg])
        mask = allbeam == beam
    ...
    def addVelPlot(self, data_dict, beam, title, vel_max=200, vel_step=25, xlabel='Time UT'):
        ...
        # gate = np.concatenate([arr for arr in data_dict['gate']])
        # allbeam = np.concatenate([arr for arr in data_dict['beam']])
        # flags = np.concatenate([arr for arr in data_dict['vel']])
    ...
    #     ax.pcolormesh(mesh_x, mesh_y, masked_colormesh, edgecolors='None', cmap=cmap, norm=norm)
    #     if label_clusters:
    ...
    def _create_colormesh(self, ax, time, gate, flags, mask, bounds, cmap, xlabel='', label_clusters=False):
        import matplotlib.dates as mdates
        import matplotlib as mpl
        ...
        # Настройка осей
        ax.xaxis.set_major_formatter(DateFormatter('%H:%M'))
        hours = mdates.HourLocator(byhour=range(0, 24, 2))  # Каждые 2 часа
        ax.xaxis.set_major_locator(hours)
        ax.set_xlabel(xlabel)
        ax.set_ylabel('Range gate')
        
        # Рисуем данные как scatter plot (точки) вместо pcolormesh
        # Это гарантирует, что каждая точка будет видна
        ...
        # Рисуем scatter с очень маленьким размером точки, чтобы они сливались в линии
        # s=0.5 делает точки очень мелкими, но видимыми при зуме
        scatter = ax.scatter(t_masked, g_masked, c=f_masked, 
        ...
        #             continue
        #         flag_mask = (flags == f) & mask
        #         if np.sum(flag_mask) == 0:
        #             continue
        #         g = gate[flag_mask].astype(int)
        #         t_c = time[flag_mask]
        #         if len(t_c) > 0:
        #             # Берём медианное время для подписи
        #             m = len(t_c) // 2
        #             ax.text(t_c[m], g[m], str(int(f)), 
...
class FanPlot:
    ...
    def add_axis(self, fig, subplot):
        ...
        theta_ticks = self.theta_ticks[:self.nbeam]
        plt.thetagrids(theta_ticks, range(self.nbeam))
        return ax
