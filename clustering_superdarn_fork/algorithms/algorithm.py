...
class Algorithm(object):
    ...
    def plot_rti(self, beam, threshold, vel_max=200, vel_step=25, show_fig=True, save_fig=False):
        ...
        for b in beams:
            ...
            if save_fig:
                plot_date = self.start_time.strftime('%Y%m%d').replace(":", ".")
    ...
    def plot_fanplots(self, start_time, end_time, vel_max=200, vel_step=25, show_fig=True, save_fig=False):
        ...
        if scan_start is None:
            scan_start = 0
        if scan_end is None:
            scan_end = len(self.data_dict['time']) - 1
        # Create the fanplots
        ...
        if save_fig:       # Only create the directory path if savePlot is true
            ...
            base_filepath = self._get_plot_path(alg, 'fanplot') + '/' + filename.replace(":", ".")
        else:
            base_filepath = None
    ...
    def _classify(self, threshold):
        ...
        vel = np.concatenate([np.abs(arr) for arr in self.data_dict['vel']])
        t = np.concatenate([arr for arr in self.data_dict['time']])
        wid = np.concatenate([np.abs(arr) for arr in self.data_dict['wid']])
